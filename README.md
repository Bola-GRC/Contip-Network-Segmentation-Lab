...
Scenario 2 demonstrates how ACLs can be modified to enforce tighter network segmentation without disrupting essential infrastructure services.

## Network Topology

The lab uses three separate `/24` networks for the IT, HR, and Sales departments. A Cisco 2911 router provides inter-subnet routing between the three networks.
...
DHCP relay is configured on the HR and Sales router interfaces using `ip helper-address 192.168.10.2`, allowing clients on remote subnets to obtain IP configuration from the centralized DHCP server.
...
A centralized DHCP server at `192.168.10.2` provides IP addressing for the IT, HR, and Sales networks.
...
The centralized DNS server is hosted at `192.168.10.3`.

The following DNS A records were created:
...
The first security policy restricts HR from accessing the IT network while maintaining access to required DHCP and DNS services.

Sales remains permitted to communicate with the IT network.

### Scenario 1 ACL Logic

* Permit HR access to the DHCP service using UDP port 67.
* Permit HR access to DNS using UDP port 53.
* Permit HR access to DNS using TCP port 53.
* Deny all other HR traffic destined for the IT subnet.
* Permit remaining traffic.

The complete intended ACL configuration is available here:
...
HR is unable to reach an IT workstation, confirming that the access restriction is working as intended.

![HR to IT Blocked](screenshots/06-Scenario-1-HR-to-IT-Blocked.png)

### Sales to IT Test

Sales successfully reaches the IT network, confirming the original business requirement.
...
Both HR and Sales are prevented from accessing general IT resources while retaining access to approved DHCP and DNS infrastructure services.

### Scenario 2 ACL Logic

* Permit HR access to approved DHCP and DNS services.
* Deny HR access to other IT resources.
* Permit Sales access to approved DHCP and DNS services.
...
The ACL match counters confirm that traffic from both HR and Sales matched their corresponding deny statements.

![Scenario 2 ACL](screenshots/08-Scenario-2-ACL-Verification.png)

### HR to IT Test

HR is unable to reach an IT workstation.

![Scenario 2 HR Blocked](screenshots/09-Scenario-2-HR-to-IT-Blocked.png)

### Sales to IT Test

Sales is also unable to reach an IT workstation under the enhanced segmentation policy.
...
The HR client successfully resolves `dns.contip.com` using the DNS server at `192.168.10.3`.
...
The Cisco Packet Tracer lab files for both scenarios are available below:
...
The verification screenshots demonstrate the access-control behavior and testing performed during the lab.
